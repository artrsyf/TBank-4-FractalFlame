...
case class FutureImageProcessor(
    ...
    def prepareImage: Future[FutureImageProcessor] = {
        val tasks: Seq[Future[DefaultImage]] = (0 until sampleCount).map { _ =>
            Future {
                ...
                (-20 until iterationCount).foldLeft(image -> initialPoint) { case ((img, currentPoint), step) => 
                    ...
                    val transformedPoint = nonLinearTransformations.foldLeft(linearTransformedcurrentPoint) { (currentPoint, nonLinearTransformation) =>
                    ...
                    if step < 0 then
                        ...
                        val xPixel = xRes - ((img.yMax - transformedPoint.x) / (img.xMax - img.xMin) * xRes).toInt
                        val yPixel = yRes - ((img.yMax - transformedPoint.y) / (img.yMax - img.yMin) * yRes).toInt
                        ...
                        if !img.isValidPixel(xPixel, yPixel) then
                            ...
                            case None =>
                                Pixel(xPixel, yPixel, transformation.colorCoeff._1, transformation.colorCoeff._2, transformation.colorCoeff._3, 1)
    ...
    private def correctImage(): DefaultImage = 
        ...
        val correctedImage = image.pixelRepo.getPixels.foldLeft(image) { (currentImage, pixel) =>
    ...
    def renderImage(format: String, path: String, applyCorrection: Boolean): Future[Unit] = {
        ...
        val tasks: Seq[Future[Unit]] = symmetrizedImage.pixelRepo.getPixels.map { pixel =>
            Future {
                if pixel.hitCount > 0 then
