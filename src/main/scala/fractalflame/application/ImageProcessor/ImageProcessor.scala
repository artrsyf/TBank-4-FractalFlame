...
case class ImageProcessor(
    ...
    def prepareImage: ImageProcessor = 
        ...
        val preparedImage = (0 until sampleCount).foldLeft(image) { (currentImage, _) => 
            val initialPoint = Point(
                random.between(image.xMin, image.xMax), 
                random.between(image.yMin, image.yMax)
            )

            (-20 until iterationCount).foldLeft((currentImage, initialPoint)) { case ((img, currentPoint), step) =>                 
    ...
    private def correctImage(): Image = 
        ...
        val correctedImage = image.getPixels.foldLeft(image) { (currentImage, pixel) =>
